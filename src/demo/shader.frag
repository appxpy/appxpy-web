#include  "../shaders/lygia/math/parabola.glsl"
...
void main() {
    ...
    vec2 uv = (1.0 * gl_FragCoord.xy - iResolution.xy) / min(iResolution.x, iResolution.y);
    ...
    vec3 color = vec3(.03)/abs(sin(slowedTime-uv.y-uv.x));
    gl_FragColor.rgb = color + vec3(noise);
    gl_FragColor.a = max(sqrt(color.r), .5);
